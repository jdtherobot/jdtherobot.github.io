...
![Vacation packages grid](../assets/screenshots/vacation-packages-grid.jpg)

**Package drill-in — add-on excursions with running bag total**
![Italy excursions](../assets/screenshots/vacation-excursions-italy.jpg)

**My REST endpoint at work — the `purchase` call returns `{"orderTrackingNumber": …}` with no network error**
![DevTools Network tab showing the purchase API response](../assets/screenshots/vacation-booking-api-network.jpg)

**Purchase confirmation (tracking number generated by my checkout service)**
![Purchase complete](../assets/screenshots/vacation-booking-purchase-complete.jpg)

**Normalized schema verified in MySQL (`cart_items`, with the full relational model in the tree)**
![MySQL cart_items table](../assets/screenshots/vacation-booking-db-cart-items.jpg)

**Many-to-many mapping proven — the `excursion_cartitem` join table populated after a multi-excursion order**
![MySQL excursion_cartitem join table](../assets/screenshots/vacation-booking-db-excursion-join.jpg)
